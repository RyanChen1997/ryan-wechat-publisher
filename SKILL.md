...
|------|------|
| 1 | 解析文章（md / word） |
| 2 | 结构确认：梳理文章层级（`#` → `1.`、`##` → `1.1`）编号发给用户，同时完成样式丰富化与图片注释（只写进 `structured.md`，消息不变，见 `references/workflow-detail.md` 第 2 步）；文案见 `references/confirmation-dialogs.md` |
...
> [!tip] 第 2 步：结构梳理 = 标题层级 + 样式梳理
> 原文是零格式长文时（尤其 Word 转来的），排出来只剩标题样式。梳理层级的同时做两件事，只写进 `structured.md`：
> **样式丰富化** —— 给重点句加格式：加粗 `**…**`、斜体 `*…*`、引用块 `> `、行内代码、列表（原文明确是并列短句时）；
> **图片注释** —— 逐张查看图片，把图注写进图片语法内部（`![[img.png|与内容相符的图注]]`），文件名式/无意义注释替换掉。
> 两件事都**不影响发给用户的确认消息**（仍是节点 1 的层级结构）。规则与克制标准见 `references/workflow-detail.md` 第 2 步。
> 逐字保留原文：只包格式标记，不改写、不概括、不删减、不新增正文文字。
...
- 不改写、不增删、不重排文章内容，除非用户明确要求（例外：第 2 步按规范加的格式标记与图片注释 —— 只包格式、图注仅限图片语法内部，正文文字不动）
